...
基于 [README.md](README.md)，构建一个三区布局的弹道仿真桌面级 Web 界面：左侧参数输入区、右侧 3D 弹道（Cesium）、底部曲线（ECharts）+ 飞行段表格。支持选择 3 个火箭（CZ-2D / CZ-4B / CZ-4C），加载 `public/templates/` 中对应 JSON，调用真实 webapi 计算/优化，并可视化结果。
...
1. 脚手架 + 主题 + 布局骨架
2. 参数表单（用 `public/templates/` 模板 JSON 驱动）
3. 接入 API 与曲线/表格
